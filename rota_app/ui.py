...
def generate(state, store):
    ...
    if solver_errors:
        ...
        st.success(f"Solver input ready: {len(data.consultants)} consultants and {len(data.weeks)} weeks.")
...
def review(state, store):
    ...
    grid = frame.pivot_table(index="week_commencing", columns="Duty", values="Consultant", aggfunc=lambda x: " / ".join(x), fill_value="")
    ...
    for consultant in active(state):
        ...
        summary.append({"Consultant": consultant["name"], "T actual / target": f"{sum(own.Duty == 'T')} / {target['t']}",
                        "C days actual / target": f"{sum(2 if d == 'C1' else 3 if d == 'C2' else 0 for d in own.Duty)} / {target['c']}",
