...
def _weeks(start: date, end: date, bank_holidays: set[date]) -> list[RotaWeek]:
    ...
    while cursor <= end:
        ...
        c1_dates = tuple(cursor + timedelta(days=n) for n in (0, 1) if cursor + timedelta(days=n) not in bank_holidays)
        c2_dates = tuple(cursor + timedelta(days=n) for n in (2, 3, 4) if cursor + timedelta(days=n) not in bank_holidays)
        c_intervals = {
            Duty.C1: tuple(Interval(datetime.combine(day, EIGHT_AM), datetime.combine(day + timedelta(days=1), EIGHT_AM)) for day in c1_dates),
            Duty.C2: tuple(Interval(datetime.combine(day, EIGHT_AM), datetime.combine(day + timedelta(days=1), EIGHT_AM)) for day in c2_dates),
        }
        result.append(RotaWeek(
            ...
            c_intervals=c_intervals, c_dates={Duty.C1: c1_dates, Duty.C2: c2_dates},
...
def build_solver_input(state: dict) -> SolverInput:
    ...
    for item in state.get("bank_holidays", []):
        raw = item.get("date") if isinstance(item, dict) else item
        try:
            holiday = date.fromisoformat(raw)
        except (TypeError, ValueError) as exc:
            raise InputBuildError(f"Bank holiday {raw!r} is not a valid date.") from exc
        if not start <= holiday <= end:
            raise InputBuildError(f"Bank holiday {holiday} falls outside the rota period.")
        if holiday.weekday() > 4:
            raise InputBuildError(f"Bank holiday {holiday} must be a Monday-to-Friday date.")
        if holiday in bank_holidays:
            raise InputBuildError(f"Bank holiday {holiday} is entered more than once.")
        bank_holidays.add(holiday)
    ...
    weeks = _weeks(start, end, bank_holidays)
    ...
    return SolverInput(start, end, consultants, absences, weeks, tuple(sorted(bank_holidays)), preferences)
