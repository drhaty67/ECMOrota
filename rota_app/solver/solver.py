        ...
        for w, week in enumerate(data.weeks):
            for duty in Duty:
                if engine.value(x[c, w, duty]):
                    assignments.append(Assignment(week.start, duty.value, consultant.name, 1.0 if duty == Duty.WEEKEND else 0.0))
            if engine.value(half_a[c, w]):
                assignments.append(Assignment(week.start, "HALF_A", consultant.name, 0.5))
            if engine.value(half_b[c, w]):
                assignments.append(Assignment(week.start, "HALF_B", consultant.name, 0.5))
    for w, week in enumerate(data.weeks):
        for duty in (Duty.C1, Duty.C2):
            if engine.value(variables["vacancy"][w, duty]):
                assignments.append(Assignment(week.start, duty.value, "VACANCY", 0.0))
