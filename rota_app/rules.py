...
HARD_CONSTRAINTS = [
    ...
    "Bank holidays have T cover but no C cover; the affected C1/C2 block earns credit only for its remaining non-bank-holiday days.",
    "Annual leave, study leave and NOC are strict hard exclusions: no overlapping C1, C2, T, full-weekend or split-weekend duty may be assigned or saved.",
    ...
    "The strict whole-block solver always runs first. If explicitly enabled and strict scheduling is infeasible, T and C blocks may be split by day while targets and every other hard rule remain enforced.",
]
...
def readiness(state: dict) -> list[dict]:
    ...
    holiday_dates = [item.get("date") if isinstance(item, dict) else item for item in state.get("bank_holidays", [])]
    try:
        parsed_holidays = [date.fromisoformat(value) for value in holiday_dates]
        bad_holidays = [value for value in parsed_holidays if not start <= value <= end or value.weekday() > 4]
        holiday_ok = len(parsed_holidays) == len(set(parsed_holidays)) and not bad_holidays
    except (TypeError, ValueError):
        parsed_holidays, holiday_ok = [], False
    checks.append({"ok": holiday_ok, "label": "Bank holidays", "detail": f"{len(parsed_holidays)} weekday bank holiday(s) configured." if holiday_ok else "Bank holidays must be unique weekdays inside the rota period."})
