...
def unavailable(data: SolverInput, consultant: str, week_index: int, component: str) -> bool:
    ...
    elif component in (Duty.C1.value, Duty.C2.value):
        intervals = week.c_intervals[Duty(component)]
    else:
...
def pre_solver_validation(data: SolverInput) -> list[str]:
    ...
    required_c_days = sum(len(week.c_dates[duty]) for week in data.weeks for duty in (Duty.C1, Duty.C2))
    ...
    if total_c_days > required_c_days:
        errors.append(f"C-day targets exceed coverage: targets total {total_c_days}; non-bank-holiday C coverage provides {required_c_days} C days.")
    else:
        gap = required_c_days - total_c_days
        vacancy_sizes = [len(week.c_dates[duty]) for week in data.weeks for duty in (Duty.C1, Duty.C2) if week.c_dates[duty]]
        attainable_gaps = {0}
        for size in vacancy_sizes:
            attainable_gaps |= {value + size for value in tuple(attainable_gaps)}
        representable = gap in attainable_gaps
        if not representable:
            errors.append(f"C-day vacancy gap {gap} cannot be represented by the available whole or bank-holiday-shortened C blocks.")

    for duty in (Duty.C1, Duty.C2, Duty.T):
        for week in data.weeks:
            if duty in (Duty.C1, Duty.C2) and not week.c_dates[duty]:
                continue
    ...
    for consultant in data.consultants:
        ...
        for week in data.weeks:
            ...
            if not unavailable(data, consultant.name, week.index, Duty.C1.value):
                attainable_c_days[consultant.name].update(value + len(week.c_dates[Duty.C1]) for value in prior)
            if not unavailable(data, consultant.name, week.index, Duty.C2.value):
                attainable_c_days[consultant.name].update(value + len(week.c_dates[Duty.C2]) for value in prior)
        if consultant.c_day_target not in attainable_c_days[consultant.name]:
            errors.append(f"{consultant.name}: C-day target {consultant.c_day_target} is unattainable from the available standard and bank-holiday-shortened C blocks.")
    return errors
...
def validate_solution(data: SolverInput, assignments: list) -> list[str]:
    ...
    for week in data.weeks:
        for duty in ("C1", "C2", "T"):
            expected = 0 if duty in ("C1", "C2") and not week.c_dates[Duty(duty)] else 1
            if len(by_week_duty[(week.start, duty)]) != expected:
                errors.append(f"{week.start} {duty}: expected {expected} assignment(s).")
    ...
    for consultant in data.consultants:
        ...
        c_actual = sum(row.c_day_credit for row in rows)
