...
2. Open **SQL Editor**, run `supabase/schema.sql`, then `supabase/integration_v2.sql`, `supabase/bank_holidays_v3.sql`, `supabase/flexible_fallback_v4.sql`, and `supabase/audit_safe_configuration_v5.sql` in that order.
...
- Optionally run a clearly labelled split-duty fallback only after strict whole-block scheduling is infeasible. Four T days equal one T-block target; C continues to be counted in days. The fallback minimises fragmentation and preserves all other hard constraints.
...
- Save leave/NOC and other configuration through an audit-safe RPC that never deletes solver runs, drafts or assignments referenced by historical records.
...
Strict-first fallback tests confirm that a feasible strict rota is never replaced, an infeasible strict rota can use split duties when explicitly enabled, and fractional T credits still reconcile exactly to integer targets.

## Schema migration note

The inherited schema models one selected period per workspace. Its original `save_rota_state` function deleted run/assignment history; `audit_safe_configuration_v5.sql` replaces it with an audit-safe compatibility wrapper and introduces `save_rota_configuration`. Multi-period browsing remains a known inherited UI/schema limitation.
