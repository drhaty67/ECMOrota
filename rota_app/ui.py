...
from .service import generate_draft, prepare, prepare_flexible
...
def persist(store, state):
    try:
        ...
        st.error(str(exc))
        # Restore the last committed state so a failed write cannot leave a
        # duplicate unsaved form entry in the active Streamlit session.
        try:
...
def generate(state, store):
    ...
    allow_fallback = st.checkbox(
        "If the strict whole-block rota is infeasible, allow split T and C duties",
        value=False,
        help="The strict solver always runs first. The fallback retains targets, leave/NOC exclusions, weekend rules and adjacency rules, but may divide T and C blocks by day between consultants.",
    )
    _, flexible_errors = prepare_flexible(state) if allow_fallback else (None, [])
    for c in checks: (st.success if c["ok"] else st.error)(f"{c['label']}: {c['detail']}")
    if solver_errors:
        (st.warning if allow_fallback and not flexible_errors else st.error)(
            "The strict block structure is not feasible; the split-duty fallback is available." if allow_fallback and not flexible_errors
            else "Pre-solver validation failed."
        )
    ...
    disabled = bool(solver_errors) and (not allow_fallback or bool(flexible_errors))
    if flexible_errors:
        for message in flexible_errors: st.error(f"Flexible fallback: {message}")
    if st.button("Generate Draft", type="primary", disabled=disabled):
        with st.status("Running OR-Tools…", expanded=True) as status:
            result = generate_draft(state, store, allow_flexible_fallback=allow_fallback)
            if result["errors"]:
                ...
                mode = result.get("solve_mode", "STRICT")
                st.success(f"Draft {result['draft_id']} saved using {mode.replace('_', ' ').title()} with {len(result['assignments'])} normalized assignments.")
                st.session_state.rota_state = store.load()
                st.rerun()
...
def review(state, store):
    ...
    def display_assignment(row):
        if row["Duty"] not in ("C1", "C2", "T"): return row["Consultant"]
        dates = row.get("duty_dates") or []
        days = [date.fromisoformat(value).strftime("%a") for value in dates]
        if row["Duty"] == "T":
            credit = float(row.get("t_block_credit") or (1 if not dates else len(days) / 4))
            return f"{row['Consultant']} ({'/'.join(days) if days else 'full'}; {credit:g} block)"
    ...
    frame["T Credit"] = frame.apply(lambda row: float(row.get("t_block_credit") or (1 if row["Duty"] == "T" else 0)), axis=1)
    ...
    for consultant in active(state):
        ...
        summary.append({"Consultant": consultant["name"], "T actual / target": f"{own['T Credit'].sum():g} / {target['t']}",
    ...
    flexible = bool(frame.get("flexible", pd.Series(dtype=bool)).fillna(False).any())
    (st.warning if flexible else st.success)(
        "Flexible fallback draft: T/C structures were split, but targets and all remaining hard constraints passed validation."
        if flexible else "Strict whole-block draft passed validation with no hard-rule violations."
    )
