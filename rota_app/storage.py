...
class SupabaseStore:
    ...
    def load(self) -> dict:
        try:
            ...
            if drafts:
                ...
                rows = self.client.table("assignments").select("consultant_id,week_commencing,assignment_type,weekend_credit,c_day_credit,duty_dates").eq("draft_id", chosen["id"]).execute().data or []
    ...
    def get_draft(self, draft_id: str) -> dict:
        try:
            ...
            draft["assignments"] = self.client.table("assignments").select("consultant_id,week_commencing,assignment_type,weekend_credit,c_day_credit,duty_dates").eq("draft_id", draft_id).execute().data or []
            return draft
        except Exception as exc:
